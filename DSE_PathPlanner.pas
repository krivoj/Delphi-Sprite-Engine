...
type
  ...
  published
    ...
    property NoDiagonal: boolean read FNoDiagonal write FNoDiagonal;
...
(*
         ...
         if  (Abs (TMapLocation(FromState).X - TMapLocation(ToState).X)   = 1)
         and (Abs (TMapLocation(FromState).Y - TMapLocation(ToState).Y)   = 1)
           then  begin
              if Odd(TMapLocation(FromState).Y) then begin      // se pari e passa per forza a dispari
                Result:= Result + 4;
              end;
           end;
...
begin
     ...
     begin
          ...
          //GG non si muove in diagonale
          if Owner.NoDiagonal then begin
          if abs(X  - (X  + XPathDirection[TPathDirection(Index)])) + abs(Y  - (Y  + YPathDirection[TPathDirection(Index)])) > 1 then begin
            Result := nil;
            exit;
            end;
          end;
